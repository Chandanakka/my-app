...
class FormDataExample extends Component {
    ...
    ;
        ...
        fetch('http://192.168.0.179:8443/AccountsReceivable', {
            ...
            body: formData
                ...
                .then(response => {
                    this.setState({message: 'All Good-1!'});
                    if (!response.ok) {
                        this.setState({message: 'All is not Good-1!' + response.toString()});
                        throw new Error('');
                    }
                    return response.toString();
                })
                .then(data => {
                    this.setState({message: 'All Good - 2; Inserted Account Receivable!!'});
                })
                .catch(error => {
                    console.error('Fetch error:', error);
                    this.setState({message: 'All is not Good-2!' + error});
                    console.error('Fetch error:', error);
                    this.setState({message: 'All is Good-3!' + error});
                });
    }
    ;
            handleClearSubmit = (e) => {
        ...
        return (
                <div className="accounts-receivables ">
                    <div className="transaction-form">
                        <div className="accounts-receivables"><h2>ACCOUNTS RECEIVABLES - MM</h2></div>
